...
entity svm_speech_30_Mdl_BinaryLearners_RAM_AUTO_1R1W is 
    generic(
        MEM_TYPE        : string    := "auto"; 
        DataWidth       : integer   := 64; 
        AddressWidth    : integer   := 9;
        AddressRange    : integer   := 430;
        COL_WIDTH       : integer := 8;
...
architecture rtl of svm_speech_30_Mdl_BinaryLearners_RAM_AUTO_1R1W is 
...
begin 
    if (clk'event and clk = '1') then
        if (ce1 = '1') then 
            ...
            for i in 0 to NUM_COL - 1 loop
                if (we1(i) = '1') then
                    ram(CONV_INTEGER(address1_tmp))((i + 1) * COL_WIDTH - 1 downto i * COL_WIDTH) := d1((i + 1) * COL_WIDTH - 1 downto i * COL_WIDTH); 
                end if;
            end loop;
        end if;
    end if;
